...
- Spread paper trading defaults to disabled with `PAPER_SPREAD_TRADING_ENABLED=false`. Spread markets may still be discovered, mapped, scored, and displayed as diagnostics, but they return `no_trade_spread_trading_disabled` unless the env explicitly enables spread paper trading after parser validation.
