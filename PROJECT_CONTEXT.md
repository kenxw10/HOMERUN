...
- Added optional `refresh_schedule=true/false` to feature sync endpoints. By default, module-specific syncs skip schedule hydration when target-date games already exist and hydrate safely when the slate is missing.
...
- `pybaseball` remains optional and is not added as a production dependency in this PR. `/v1/model/sources/status` reports `pybaseball_available` and `advanced_public_stats_status`; schedule-derived team/pitcher/bullpen rows remain partial rather than being labeled as complete advanced public stats.
