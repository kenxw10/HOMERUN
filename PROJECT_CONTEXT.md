...
- Default policy version is `pr3x_paper_risk_governance_v1` with `PAPER_RISK_GOVERNANCE_ENABLED=true`. Drawdown halt defaults are `PAPER_DRAWDOWN_HALT_ENABLED=true`, `PAPER_DRAWDOWN_HALT_THRESHOLD_ABS=50.00`, and `PAPER_DRAWDOWN_HALT_THRESHOLD_PCT=0.10`. A halt blocks new paper trades only; it does not close positions or alter settlement.
