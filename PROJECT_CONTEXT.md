...
### PR3u - Family/Scope Probability Adapters

- Candidate generation now routes each side-aware candidate through explicit PR3u probability adapter metadata for `full_game_total`, `first_five_total`, `full_game_winner`, `first_five_winner`, `full_game_spread`, and `first_five_spread`.
- Adapters record compact candidate-level metadata: adapter key/version/policy version, family, scope, rationale, calibration hook/version, feature policy version, and bounded diagnostics. `/v1/model/predictions`, `/v1/candidates/today`, candidate-sweep summaries, and compact dashboard candidate diagnostics expose only compact adapter fields/counts, not raw feature or scoring blobs.
- PR3u separates full-game bullpen-aware policy metadata from first-five starter-heavy policy metadata and preserves PR3s exposure semantics for totals, winner NO contracts, first-five ties, and spread complement safety. Ambiguous spread complements are flagged instead of treated as trusted alternate probabilities.
...
### PR3x - Paper Risk Governance and Operator Controls

- Candidate generation now runs a compact PR3x paper risk-governance pass after PR3t live-like selection and PR3w hardening, before legacy caps, fixed-risk sizing, and paper trade creation.
- All scored candidates remain persisted for governance/backtesting and dry-run validation. PR3x only decides whether a selector-approved candidate may continue toward paper trade creation; it does not make any rejected candidate tradable.
- Default policy version is `pr3x_paper_risk_governance_v1` with `PAPER_RISK_GOVERNANCE_ENABLED=true`. Legacy drawdown config remains visible as `PAPER_DRAWDOWN_HALT_ENABLED=true`, `PAPER_DRAWDOWN_HALT_THRESHOLD_ABS=50.00`, and `PAPER_DRAWDOWN_HALT_THRESHOLD_PCT=0.10`, but PR4a makes drawdown report-only in paper observation and reserves hard enforcement for future live-readiness metadata.
- Family controls default to enabled for full-game/first-five totals and winners, paper-only for full-game spreads behind the existing trusted audit gate, and shadow-only for first-five spreads. PR3x also enforces concept-cluster, same-game, alternate-line, and low-price/tail caps.
- Candidate and paper-trade rows now store nullable compact risk fields: policy version, enabled/status/decision/reason, family and cap statuses, drawdown status, approved-before/after flags, shadow/blocked flags, and risk rank/score. `/v1/model/predictions`, `/v1/candidates/today`, candidate-sweep summaries, compact dashboard diagnostics, dashboard position rationale, and `/v1/system/status` expose bounded PR3x metadata without raw payloads.
- Migration `0017_pr3x_risk_governance.py` adds nullable PR3x columns to `model_candidates` and `paper_trades`.
- PR3x does not change model probability math, EV thresholds, PR3t selector thresholds, PR3w hardening math, settlement, source ingestion, cron cadence, market discovery, WebSocket behavior, live execution, credentials, sportsbook/Odds API scope, team totals, umpire factors, or MVE/multivariate markets.

### PR4a - Settlement, Accounting, and Drawdown Audit Hardening

- PaperTrade rows now retain compact reconstruction metadata for selected trades: exposure taxonomy, PR3u probability adapter fields, PR3w hardening fields, calibration status, PR3t selector fields, PR3x risk-governance fields, sizing/economics, and settlement audit fields.
- Settlement writes a stable `settlement_audit_key` / `settlement_idempotency_key`, formula version, explicit formula, MLB source identifiers, checked/resolved timestamps, status, outcome, skip/error reason, payout, and fee adjustment to the trade row. The existing `settlements` table remains the unique settlement ledger, and settlement reruns report already-settled trades without duplicating settlement rows or realized P/L.
- Supported settlement formulas remain bounded to validated paper families: full-game winner, first-five winner, full-game/first-five totals, first-five spread, and full-game spread only when cached PR3o.1 trusted-audit metadata is present. Untrusted spread rows remain open with explicit skip reasons.
- Dashboard summary now exposes active-epoch accounting fields for starting balance/current equity, settled P/L, unrealized open P/L, paid and estimated fees, open cost, open mark value, open trade count, and settled trade count. Position rows include compact model, risk, exposure, selector, and settlement audit payloads when present.
- PR4a changes drawdown behavior in the current paper-observation posture. With `PAPER_TRADING=true`, `LIVE_TRADING_ENABLED=false`, and the kill switch on, drawdown is diagnostic/report-only: the system records `drawdown_would_have_halted` and `drawdown_halt_enforced=false` instead of applying `no_trade_risk_drawdown_halt`. Future live-readiness metadata uses `starting_bankroll_minus_150` and a `$150` live halt threshold for new entries only, but this PR does not add live order execution.
- Migration `0018_pr4a_settlement_audit.py` / revision `0018_pr4a_settlement_audit` adds nullable audit columns to `paper_trades`.
- PR4a does not change selector behavior, EV thresholds, model probability math, risk caps other than paper drawdown enforcement posture, candidate-sweep cache-only behavior, source ingestion, cron cadence, market discovery, WebSocket behavior, full-game spread audit requirements, live execution, credentials, sportsbook/Odds API scope, team totals, umpire factors, or MVE/multivariate markets.

### PR4b - Final Live-Readiness Audit Pack

- Added read-only readiness policy `pr4b_final_readiness_audit_pack_v1` with `GET /v1/readiness/audit-pack`. The pack summarizes bot mode, safety gates, validated PR3s/PR3t/PR3u/PR3v/PR3w/PR3x/PR4a components, current accounting, model governance, candidate pipeline evidence, settlement audit evidence, spread audit evidence, live blockers, and an operator checklist.
- `/v1/system/status` and `/v1/dashboard/summary` now include a compact readiness summary with `paper_observation_status`, `live_readiness_status`, safety gates, blocker counts, and checklist counts. The compact summary intentionally excludes raw payloads, feature blobs, scoring rationale, orderbook data, and full job results.
- Live readiness remains hard-blocked: `live_readiness_status=blocked_for_live`, `live_enabled=false`, and `operator_review_required=true`. Required next work is a separate explicit live-readiness design PR, not a config flip.
- PR4b is read-only and adds no migration. It does not change candidate selection, EV thresholds, risk caps, settlement behavior, model math, cron cadence, source ingestion, market discovery, WebSocket behavior, full-game spread audit requirements, live execution, credentials, sportsbook/Odds API scope, team totals, umpire factors, or MVE/multivariate markets.

### PR4b.1 - Settlement Job Memory and Stale-Run Hardening

- Settlement job status now has compact stale-running diagnostics. A settlement run stuck in `running` for more than 30 minutes is surfaced as `stale_running` in dashboard/readiness summaries and is marked `failed_stale` before the next settlement job starts.
- Settlement sync now uses bounded target-date batches for candidate outcome labeling, open-trade settlement, and already-settled audit backfill. Results include compact batch-limit and warning fields instead of raw trade/job payloads.
- The Railway settlement catchup command is `python -m app.jobs.runner --job settlement --target-date yesterday_et`. That runner now emits structured JSON cron logs for startup, target ET/UTC window, lock/skipped status, stale recovery, settlement query scope, batch caps/counts, balance snapshot action, completion, and caught exceptions.
- `/v1/system/status`, `/v1/dashboard/summary`, and `/v1/readiness/audit-pack` expose compact settlement job warnings while preserving PR4b's raw-payload-free readiness posture.
- No migration or environment change is required. PR4b.1 does not change settlement formulas, idempotency keys, candidate selection, EV thresholds, risk caps, model math, cron cadence, source ingestion, market discovery, WebSocket behavior, live execution, credentials, sportsbook/Odds API scope, team totals, umpire factors, or MVE/multivariate markets.

### PR4c - One-Way Conservative Total-Tail Hardening

- Default probability hardening policy is now `pr4c_one_way_conservative_total_tail_hardening_v1`; the PR3w policy name remains available for historical rows and tests.
- For `full_game_total` and `first_five_total` `deep_alternate` / `tail` rows, hardening is one-way conservative: the selection probability cannot be increased above `probability_before_hardening` by the central-anchor formula.
- Total tail/deep-alternate rows with `probability_raw_adapter` now receive a raw-adapter inflation cap: `min(raw + configured_abs_lift, raw * configured_multiplier)`. Missing raw adapters do not crash candidate generation; total tails with missing raw adapter remain shadow-only.
- PR4c tightens only total deep/tail selector thresholds. Total tails require at least `PAPER_TOTAL_TAIL_MIN_NET_EV=0.20` and `PAPER_TOTAL_TAIL_MIN_PROB_EDGE=0.12`; total deep alternates require stricter standard and low-price thresholds. Central totals, near alternates, winners, spreads, risk caps, settlement, cron cadence, source ingestion, WebSocket behavior, market discovery, and live execution remain unchanged.
- Candidate/prediction/dashboard metadata uses existing compact scalar fields: before/after hardening, raw adapter probability, reason/status, line-class policy, shadow/block flags, and selector threshold profile/policy. No raw payloads, feature blobs, scoring rationale blobs, orderbook data, or unbounded diagnostics were added.
- No migration is required. Existing open paper positions are not rewritten or force-closed; PR4c applies to future candidate scoring/selection only. Safety posture remains paper-only, demo Kalshi, live trading disabled, kill switch on, no production credentials, no sportsbook/Odds API behavior, no team totals, no umpire factors, and no MVE/multivariate markets.

### PR4d - Observation Diagnostics and Portfolio Chart Padding

- Dashboard portfolio chart y-axis padding now scales with the visible value range and adds a minimum floor for flat/tight ranges, so active paper bankroll lines do not hug the top or bottom border. Chart range controls, adaptive x-axis labels, and compact portfolio-series handling remain unchanged.
- Dashboard open and closed position rows expose compact nullable version metadata aliases from existing trade/candidate columns: model version, calibration version, calibration policy, risk policy, economic exposure line class, and a bounded missing-metadata reason for legacy rows. No raw rationale parsing or new schema migration is required.
- Full-game spread audit freshness is now a compact diagnostic in dashboard trade policy, readiness audit packs, and system-status readiness summaries. It reports the latest spread-audit age, target date, fixed ET freshness status, stale warning, and recent full-game-spread paper activity count without loading raw audit items.
- PR4d does not change candidate selection, EV thresholds, model math, governance promotion, risk caps, settlement formulas, source ingestion, cron schedules, live execution, credentials, sportsbook/Odds API scope, team totals, umpire factors, or MVE/multivariate markets. Safety posture remains paper-only, demo Kalshi, live trading disabled, and kill switch on.

### PR4e - Spread-Audit Coverage Automation and Calibration Diagnostics

- Spread-audit job results are now coverage-aware. Each audit reports target-date mapping counts, distinct market/game counts, in-window mapping counts, before/after-window skips, coverage ratio/status, zero-checked reason, and read-only invariants (`audit_only=true`, `read_only=true`, `paper_trades_created=0`, `mapping_mutations=0`, `settlement_rows_created=0`).
- Spread-audit freshness policy `pr4e_spread_audit_coverage_freshness_v1` evaluates bounded current-day `JobRun` evidence. A later zero-work run no longer erases an earlier covered same-day run, while incomplete zero-checked or partial-coverage runs remain visible through compact dashboard/readiness/system warnings.
- The spread-audit runner emits structured JSON logs for the audit window, coverage, result counts, warning state, clean completion, and caught exceptions. Railway can run a separate short-lived audit-only cron with `python -m app.jobs.runner --job spread-audit --target-date today_et --min-time-to-start-minutes 45 --max-time-to-start-minutes 360`.
- Governance status now exposes compact PR4e calibration coverage for all six PR3v family/scope units plus bounded rolling adapter-error diagnostics for the last 1 day, last 7 days, and since the clean governance cutoff. These diagnostics read existing candidate/model metadata only and do not train, promote, or change scoring.
- Candidate-sweep remains cache-only and does not run spread audit. No migration is required. PR4e does not change candidate selection, EV thresholds, model math, governance promotion logic, risk caps, settlement formulas, source ingestion, market discovery, WebSocket behavior, live execution, credentials, sportsbook/Odds API scope, team totals, umpire factors, MVE/multivariate markets, first-five spread trading, or the trusted full-game spread gate.

### PR4e.1 - Zero-Window Spread-Audit Freshness Semantics

- Production validation on July 11, 2026 showed two successful current-day spread-audit runs with target-date spread mappings but zero in-window mappings under the 45-360 minute window. The run results had `target_date_mapping_count=6`, `in_window_mapping_count=0`, `checked=0`, `coverage_status=no_mappings_in_window`, and `zero_checked_reason=all_target_date_mappings_outside_window`, but dashboard/readiness/system status incorrectly reported `freshness_status=fresh_no_eligible_markets` with no warnings.
- Freshness policy is now `pr4e1_spread_audit_coverage_freshness_v1`. True `no_target_date_mappings` remains a valid fresh no-work state. `no_mappings_in_window` now reports `incomplete_zero_checked` with both spread-audit coverage and stale warnings when no covered current-day run exists.
- Spread-audit run results now serialize `coverage_ratio=null` whenever `in_window_mapping_count=0`, because there is no valid coverage denominator. This applies to both `no_target_date_mappings` and `no_mappings_in_window`.
- Covered current-day evidence is still preserved: if an earlier current-day run is `covered`, a later zero-window run does not erase `fresh_covered`; latest-run zero-work diagnostics stay visible through `latest_run_*` fields and `latest_covered_run_at` / `latest_covered_target_date` remain populated.
- The spread-audit runner now emits per-run `coverage_warning=true` for `no_mappings_in_window`, `zero_checked_with_eligible_mappings`, `partial_coverage`, and `unknown`, while leaving `covered` and `no_target_date_mappings` warning-free.
- PR4e.1 updates the PowerShell validation docs to use the hardcoded production backend URL, prompt for the internal API key, extract nested protected job results safely, save large responses to timestamped files, avoid interactive `foreach (...) { ... } | Format-Table` parser errors, and document the corrected expected states.
- Calibration coverage and rolling adapter-error diagnostics are intentionally unchanged. Candidate-sweep remains cache-only and independent from spread audit. No migration is required. Safety posture remains paper-only: `PAPER_TRADING=true`, `LIVE_TRADING_ENABLED=false`, `EXECUTION_KILL_SWITCH=true`, `KALSHI_ENV=demo`, no production Kalshi credentials, no live orders, no sportsbook/Odds API scope, no team totals, no umpire factors, no MVE/multivariate markets, and no first-five spread trading activation.

### PR4f - First-Five Spread Integrity and Audit Readiness

- PR4f separates first-five spread paper activation behind `PAPER_FIRST_FIVE_SPREAD_TRADING_ENABLED=false`; the broad `PAPER_SPREAD_TRADING_ENABLED` flag is not sufficient. First-five candidates and settlement require cached `trusted_audit_only` metadata with verified first-five scope, official-result-source evidence, line/complement semantics, and settlement metadata.
- The protected `first-five-spread-audit` job and `POST /v1/jobs/run/first-five-spread-audit` endpoint are bounded, read-only diagnostics. They do not run mapping sync, mutate candidates/mappings, create paper trades, write settlements, or enable trading. Freshness uses `pr4f_first_five_spread_audit_freshness_v1` and is reported separately from full-game spread audit freshness.
- `/v1/model/first-five-spread-adapter-repair-preview` is a bounded read-only classification preview. It never writes repair changes. Compact first-five audit, adapter, governance, activation-warning, and readiness fields are exposed without raw audit item arrays.
- The PR4f corrective follow-up makes preview classifications strict: nested adapter errors are grouped before any valid/repairable result, trusted first-five verification requires coherent metadata and all required evidence, and the affected target-date range is computed across the full matching candidate set even when examples are bounded.
- Candidate-sweep remains cache-only and does not run either spread audit or heavy feature/source ingestion. No migration or Railway configuration is required. Full-game spread verification/trading remains on its existing dedicated flag and PR3o.1 trusted-audit gate.
- Safety posture is unchanged: `PAPER_TRADING=true`, `LIVE_TRADING_ENABLED=false`, `EXECUTION_KILL_SWITCH=true`, `KALSHI_ENV=demo`, no production credentials, no live order path, no sportsbook/team-total/umpire/MVE scope, and no WebSocket or source-ingestion changes.
