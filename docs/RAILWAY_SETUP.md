...
- `pybaseball` is optional in the current backend requirements. `/v1/model/sources/status` reports `pybaseball_available` and `advanced_public_stats_status`; if unavailable, advanced public stat modules should be treated as degraded partial proxies.
