...
The PR3c fix3/fix4 feature pipeline uses no-key public MLB and weather sources by default. `FEATURE_SYNC_ENABLE_NETWORK_SOURCES=true` makes feature sync hydrate the MLB Stats API schedule/feed, write raw team, pitcher, bullpen, lineup, weather, park, travel, and final `mature_mlb_features_v2` snapshot rows, and expose `/v1/model/sources/status` diagnostics. If network sync is disabled, network-backed module endpoints return `validation_status=skipped_network_disabled` with zero inserted/updated rows rather than pretending ingestion succeeded. Open-Meteo weather uses `OPEN_METEO_BASE_URL=https://api.open-meteo.com/v1`; optional injury, lineup, and weather provider keys remain optional and are not required for this PR. PR3c fix4 makes schedule hydration idempotent by de-duplicating MLB `gamePk` values, updating existing `mlb_games.external_game_id` rows, and returning structured degraded JSON instead of unhandled 500s for source-ingestion errors. Feature endpoints accept optional `refresh_schedule=true/false`; module syncs skip schedule hydration by default when target-date games already exist.

`pybaseball` is currently optional and is not installed by the backend requirements. `/v1/model/sources/status` reports `pybaseball_available` as an import diagnostic and `advanced_public_stats_status` as the actual ingestion status. Until a pybaseball-backed adapter is implemented, advanced stats should report `not_ingested_pybaseball_adapter_not_implemented` if the package is present or `unavailable_pybaseball_not_installed` if absent. Team/pitcher/bullpen feature rows remain conservative MLB schedule-derived partial proxies rather than being labeled as complete advanced Statcast/Baseball Savant ingestion.
