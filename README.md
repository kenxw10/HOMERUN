...
- PR3x makes `PAPER_RISK_GOVERNANCE_ENABLED=true` the default paper risk-governance layer after the PR3t selector and before legacy caps/sizing. It enforces family, concept-cluster, same-game, alternate-line, and low-price/tail caps, and records compact risk metadata on candidates, prediction output, paper trades, and dashboard summaries. PR4a changes drawdown behavior in paper observation: drawdown is reported with `drawdown_policy_version=pr4a_paper_observation_drawdown_policy_v1`, `drawdown_would_have_halted`, and `drawdown_halt_enforced=false` instead of blocking new paper trades. Future live-readiness metadata uses a hard halt level of starting bankroll minus `$150`, but live execution remains disabled and unimplemented. It does not close positions, change model math, run source ingestion, or change cron schedules.
- PR4a adds nullable settlement/accounting audit fields to `paper_trades` so each selected paper trade can retain compact exposure, adapter, hardening, selector, risk, economics, settlement formula/source/status, and idempotency metadata. Settlement remains paper-only and idempotent; reruns report already-settled trades without double-counting P/L or duplicating settlement rows.
- PR4d keeps dashboard/readiness diagnostics compact while adding spread-audit freshness warnings and top-level position version metadata aliases from existing columns. The portfolio chart also uses dynamic y-axis padding so flat or tight bankroll ranges are readable. No migration, trading behavior, cron, source-ingestion, settlement, model, selector, risk, or live-execution change is included.
- PR4e makes spread-audit freshness coverage-aware and adds compact calibration/adapter-error coverage diagnostics. PR4e.1 uses freshness policy `pr4e1_spread_audit_coverage_freshness_v1`, treats zero-denominator coverage ratios as null, and distinguishes true no-target-date mappings from mappings that are merely outside a selected audit window. It is read-only/audit-only, requires no migration, keeps candidate-sweep cache-only, and does not change trading, settlement, model math, calibration diagnostics, selector thresholds, risk caps, source ingestion, credentials, WebSocket, or live execution.
...
.\.venv\Scripts\python.exe -m app.jobs.runner --job spread-audit --target-date today_et --min-time-to-start-minutes 45 --max-time-to-start-minutes 360
...
- `POST /v1/jobs/run/spread-audit?target_date=YYYY-MM-DD&min_time_to_start_minutes=45&max_time_to_start_minutes=360`
...
PR4e makes the protected `spread-audit` job coverage-aware and suitable for a separate short-lived audit-only Railway cron. Results include target-date and in-window mapping counts, coverage status, zero-checked reason, read-only invariants, and compact freshness evidence. PR4e.1 corrects zero-window semantics: `coverage_ratio` is null when no in-window denominator exists, true `no_target_date_mappings` can report `fresh_no_eligible_markets`, and target-date mappings outside the selected window report `incomplete_zero_checked` with coverage/stale warnings unless an earlier covered current-day run exists. Dashboard trade policy, readiness, and system status expose the compact coverage/freshness fields without raw item arrays. Governance status also includes compact calibration coverage for all six family/scope units and bounded rolling adapter-error diagnostics. Candidate-sweep remains cache-only and does not run spread audit. No migration, trading behavior, settlement formula, source-ingestion, selector, risk, model, calibration-training, WebSocket, credential, or live-execution behavior changes are included.
...
## PR3x Paper Risk Governance

PR3x adds a paper-only risk-governance pass after PR3t live-like selection and PR3w hardening, before paper trade creation. All scored candidates remain persisted for governance/backtesting, but only selector-approved candidates that also pass PR3x can reach the existing caps, fixed-risk sizing, and paper trade creation path.

The default PR3x policy (`pr3x_paper_risk_governance_v1`) applies family status controls, family caps, concept-cluster caps, same-game caps, alternate-line caps, low-price/tail caps, and active-epoch drawdown diagnostics. Full-game and first-five totals/winners remain enabled, full-game spreads remain paper-only behind the existing trusted audit gate, and first-five spreads remain shadow-only by default.

New scored candidates expose compact PR3x fields through `/v1/model/predictions`, `/v1/candidates/today`, candidate-sweep summaries, dashboard candidate diagnostics, and selected paper-trade rationale. The metadata is scalar and bounded: policy version, enabled/status/decision/reason, family/cap statuses, drawdown status, approved-before/after flags, shadow/blocked flags, and risk rank/score.

PR3x adds nullable columns to `model_candidates` and `paper_trades` with migration `0017_pr3x_risk_governance.py`. It does not change model probability math, EV thresholds, settlement, source ingestion, cron schedules, WebSocket behavior, market discovery, live execution, credentials, sportsbook/Odds API scope, team totals, umpire factors, MVE/multivariate markets, or existing paper trade lifecycle behavior.

## PR4a Settlement and Accounting Audit Hardening

PR4a makes paper settlement and dashboard accounting easier to audit without changing selector behavior, EV thresholds, risk caps, market discovery, cron cadence, source ingestion, model math, or live execution.

Paper trades now keep nullable compact audit fields for the selected candidate's probability adapter, probability hardening, calibration status, selector, risk-governance, exposure taxonomy, sizing/economics, and settlement result. Settlement writes `settlement_audit_key`, `settlement_formula_version`, `settlement_formula`, source identifiers, checked/resolved timestamps, status, outcome, skip/error reason, idempotency key, payout, and fee adjustment directly on the `paper_trades` row. Re-running settlement reports already-settled trades and does not create duplicate settlement rows or double-apply realized P/L.

Dashboard summary accounting exposes active-epoch starting balance/current equity, settled P/L, unrealized/open P/L, paid and estimated fees, open cost, open mark value, open trade count, settled trade count, and PR4a drawdown policy fields. Portfolio series remains active-epoch balance-snapshot based when usable and stays compact.
...
PR4b adds read-only readiness policy `pr4b_final_readiness_audit_pack_v1` and `GET /v1/readiness/audit-pack`. The pack summarizes paper bot mode, safety gates, validated PR3s/PR3t/PR3u/PR3v/PR3w/PR3x/PR4a evidence, current paper accounting, model governance, candidate pipeline health, settlement audit health, spread audit health, live blockers, and an operator checklist.

`/v1/system/status` and `/v1/dashboard/summary` include compact readiness summaries. They intentionally avoid raw payloads, raw features, scoring rationale blobs, orderbook data, and full job results.
