...
`pybaseball` is currently optional and is not installed by the backend requirements. `/v1/model/sources/status` reports `pybaseball_available` and `advanced_public_stats_status`. When it is unavailable, team/pitcher/bullpen feature rows remain conservative MLB schedule-derived partial proxies rather than being labeled as complete advanced Statcast/Baseball Savant ingestion.
