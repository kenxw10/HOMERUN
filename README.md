...
- PR4e makes spread-audit freshness coverage-aware and adds compact calibration/adapter-error coverage diagnostics. It is read-only/audit-only, requires no migration, keeps candidate-sweep cache-only, and does not change trading, settlement, model math, selector thresholds, risk caps, source ingestion, credentials, WebSocket, or live execution.
