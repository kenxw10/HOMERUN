...
- PR3x makes `PAPER_RISK_GOVERNANCE_ENABLED=true` the default paper risk-governance layer after the PR3t selector and before legacy caps/sizing. It blocks new paper trades on drawdown halt, enforces family, concept-cluster, same-game, alternate-line, and low-price/tail caps, and records compact risk metadata on candidates, prediction output, paper trades, and dashboard summaries. Defaults are `PAPER_RISK_GOVERNANCE_POLICY_VERSION=pr3x_paper_risk_governance_v1`, `PAPER_DRAWDOWN_HALT_ENABLED=true`, `PAPER_DRAWDOWN_HALT_THRESHOLD_ABS=50.00`, and `PAPER_DRAWDOWN_HALT_THRESHOLD_PCT=0.10`. It does not close positions, change settlement, enable live execution, change model math, run source ingestion, or change cron schedules.
